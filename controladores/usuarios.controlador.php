			...
			if(preg_match('/^(([A-Za-z0-9]+_+)|([A-Za-z0-9]+\-+)|([A-Za-z0-9]+\.+)|([A-Za-z0-9]+\++))*[A-Za-z0-9]+@((\w+\-+)|(\w+\.))*\w{1,63}\.[a-zA-Z]{2,6}$/', $_POST["ingUsuario"]) &&
				...
				if($respuesta["usuario"] == $_POST["ingUsuario"] && $respuesta["password"] == $encriptar){
					...
					echo '<script>
							swal({
								type: "error",
								title: "¡Error al ingresar, sus datos!",
								showConfirmButton: true,
								confirmButtonText: "Cerrar"
							}).then(function(result){
								if(result.value){
	...
	static public function ctrCrearUsuario(){
		...
		if(isset($_POST["nuevoUsuario"])){
			...
			if(preg_match('/^[a-zA-ZñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["nuevoNombre"]) &&
			   preg_match('/^(([A-Za-z0-9]+_+)|([A-Za-z0-9]+\-+)|([A-Za-z0-9]+\.+)|([A-Za-z0-9]+\++))*[A-Za-z0-9]+@((\w+\-+)|(\w+\.))*\w{1,63}\.[a-zA-Z]{2,6}$/', $_POST["nuevoUsuario"]) &&
				 preg_match('/^[a-zA-Z0-9]+$/', $_POST["nuevoPassword"]) &&
				 preg_match('/^[a-zA-ZñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["nuevaDescripcion"])){
	...
	static public function ctrOlvidoPassword(){
		if (isset($_POST["passEmail"])) {
			if(preg_match('/^(([A-Za-z0-9]+_+)|([A-Za-z0-9]+\-+)|([A-Za-z0-9]+\.+)|([A-Za-z0-9]+\++))*[A-Za-z0-9]+@((\w+\-+)|(\w+\.))*\w{1,63}\.[a-zA-Z]{2,6}$/', $_POST["passEmail"])){
				...
				function generarpassowrd($Longitud){
					$llaves = "";
					$pattern = "123456789abcdefghijklmnopqrstuxyz$*";
					$max = strlen($pattern)-1;

						for ($i=0; $i < $Longitud; $i++) { 
							$llaves .= $pattern{mt_rand(0,$max)}; 
						}
						return $llaves;
				}

				$nuevoPassword = generarpassowrd(11);
				$encriptar = crypt($nuevoPassword, '$2a$07$asxx54ahjppf45sd87a5a4dDDGsystemdev$');
				$tabla = "usuarios";
				$item1 = "usuario";
				$valor1 = $_POST["passEmail"];

				$res1 = ModeloUsuarios::MdlMostrarUsuarios($tabla, $item1, $valor1);
				
					if ($res1) {
						$id =  $res1["id"];
						$item2 = "password";
						$valor2 = $encriptar;
						$res2 = ModeloUsuarios::mdlActualizarUsuarioPassword($tabla, $id, $item2, $valor2);
						...
						if ($res2 == "ok") {
							...
							$mail->MsgHTML('');
							$mail->Subject = ("Recuperacion"); // Este es el titulo del email. 
							$body = ('<!DOCTYPE html>
							<head>
								<meta charset="UTF-8">
                    ...
                    swal({
                        title: "Registrado",
                        text: "¡Por favor revise la bandeja de entrada o la carpeta de SPAM de su correo electrónico '.$_POST["passEmail"].' para verificar la cuenta!",
                        type:"success",
                        confirmButtonText: "Cerrar",
												closeOnConfirm: false
												}).then(function(result) {
						...
						swal({
							  type: "error",
							  title: "Error al enviar el correo electrónico no existe en el sistema",
							  showConfirmButton: true,
							  confirmButtonText: "Cerrar",
							  closeOnConfirm: false
							  }).then(function(result) {
				...
				swal({
					  type: "error",
					  title: "Error al enviar el correo electrónico, Esta mal escrito su correo",
					  showConfirmButton: true,
					  confirmButtonText: "Cerrar",
					  closeOnConfirm: false
					  }).then(function(result) {
